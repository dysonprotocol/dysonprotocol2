...
def test_make_trade_v3_exact_out_happy(chainnet, ws_setup_env):
    dysond = chainnet[0]
    env = ws_setup_env
    ...
    assert out_b == target_out_b, f"expected credit {target_out_b}{b}, got {out_b}{b}"
    assert pre_b + out_b == post_b, f"b mismatch: pre={pre_b} out={out_b} post={post_b}"
