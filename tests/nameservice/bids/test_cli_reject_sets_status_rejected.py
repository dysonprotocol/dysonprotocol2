...
def test_cli_reject_sets_status_rejected(
    ...
    bidder_name, bidder_addr = generate_account("reject_bidder")
    ...
    # owner rejects and sets a new valuation higher than bid
    rej = dysond_bin(
        "tx",
        "nameservice",
        "reject-bid",
    ...
    assert rej.get("code", 1) == 0, rej

    st, full = _latest_bid_status_for_nft(dysond_bin, class_id, nft_id)
    assert st in (
        4,
        "BID_REJECTED",
    ), f"Expected REJECTED, got {st}. Full: {json.dumps(full, indent=2)}"
