...
func (k Keeper) AcceptBid(ctx context.Context, msg *nameservicev1.MsgAcceptBid) (*nameservicev1.MsgAcceptBidResponse, error) {
	...
	if err != nil {
		return nil, cosmossdkerrors.Wrapf(err, "failed to get active bid ID")
	}

	rec, err := k.bids.Get(ctx, bidID)
	if err != nil {
		return nil, cosmossdkerrors.Wrapf(err, "failed to get bid record")
	}

	rec.Status = nameservicev1.BidStatus_BID_ACCEPTED
	if err := k.bids.Set(ctx, bidID, rec); err != nil {
		return nil, cosmossdkerrors.Wrapf(err, "failed to update bid status")
	}

	if err := k.activeBidForNFT.Remove(ctx, collections.Join(msg.NftClassId, msg.NftId)); err != nil {
		return nil, cosmossdkerrors.Wrapf(err, "failed to remove active bid index")
	}
