...
func (k Keeper) AuctionsBySeller(ctx context.Context, req *whaleswapv1.QueryAuctionsBySellerRequest) (*whaleswapv1.QueryAuctionsBySellerResponse, error) {
	...
	results, pageRes, err := query.CollectionPaginate(ctx, k.AuctionsMap, req.Pagination, func(key uint64, value whaleswapv1.AuctionRecord) (*whaleswapv1.AuctionRecord, error) {
		if value.Seller != req.Seller {
			return nil, nil
		}
		v := value
		return &v, nil
...
func (k Keeper) AuctionsByPairPriceRange(ctx context.Context, req *whaleswapv1.QueryAuctionsByPairPriceRangeRequest) (*whaleswapv1.QueryAuctionsByPairPriceRangeResponse, error) {
	...
	results, pageRes, perr := query.CollectionPaginate(ctx, k.AuctionsBySellBid, req.Pagination, func(key collections.Triple[string, string, uint64], id uint64) (*whaleswapv1.AuctionRecord, error) {
		k1, k2, _ := key.K1(), key.K2(), key.K3()
		if k1 != req.SellDenom || k2 != req.BidDenom {
			return nil, nil
		}
